...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('winners')
		.setDescription('List leaderboard of winners within the server'),
				...
				const createMessage = (pageIndex) => {
					...
					pageItems.forEach(function (value) {
						if (userStats.indexOf(value) + 1 == 1){
							content += `🥇 - ${value.userID} | ${value.wins} \n`
						}
							...
							content += `${userStats.indexOf(value) + 1} - ${value.userID} | ${value.wins} \n`
						}
					});
					...
					.setDescription("The top 5 race win leaders")
